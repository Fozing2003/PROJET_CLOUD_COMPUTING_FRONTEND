...
// Données factices (à remplacer par une API ou un contexte global)
const postsData = [
  {
    id: "1",
    title: "La quête de la beauté et de l’esthétique et son impact sur notre travail",
    image:
      "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=800&q=80",
    description: "La recherche de la beauté a toujours été un élément essentiel...",
    content:
      "Contenu détaillé de l’article 1... vous pouvez ajouter ce que vous voulez.",
  },
  {
    id: "2",
    title:
      "Les herbiers : la vision moderne de TerraLiving sur le patrimoine botanique avec des terrariums élégants",
    image:
      "https://images.unsplash.com/photo-1529156069898-49953e39b3ac?auto=format&fit=crop&w=800&q=80",
    description: "Découvrez comment TerraLiving réinvente les herbiers traditionnels...",
    content:
      "Contenu détaillé de l’article 2... vous pouvez ajouter ce que vous voulez.",
  },
];
...
export default function EditPost() {
  ...
  if (!post) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <p className="text-gray-600 text-lg">Article non trouvé.</p>
      </div>
    );
  }
  ...
  const handleFile = (file) => {
    if (file && !file.type.startsWith("image/")) {
      setErrors({
        ...errors,
        image: "Veuillez télécharger une image (PNG, JPG, GIF).",
      });
      return;
    }
    setImageFile(file);
  ...
  const validateForm = () => {
    ...
    if (!title.trim() || title.length < 5) {
      newErrors.title = "Le titre est requis et doit contenir au moins 5 caractères.";
    }

    if (!content.trim() || content.length < 20) {
      newErrors.content =
        "Le contenu est requis et doit contenir au moins 20 caractères.";
    }
